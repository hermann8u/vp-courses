...
Un contrôleur est donc composé d'**actions**, il s'agit des fonctions ciblées par le routing et effectuant les traitements nécessaires à l'affichage de notre page.

Une action renvoie toujours une instance de Response :
- Dans l'action **list**, on se sert de la méthode **render** fournit par l'AbstractController étendu par notre classe. Cette fonction renvoie une Response avec comme contenu un template Twig traité.
- Dans l'action **show**, on instancie nous même un objet Response avec son contenu.

::: tip
Dans cet exemple, j'ai extrait les parties communes à toutes les routes de ce contrôleur dans une annotation sur la classe elle-même. Ceci permet d'éviter de se répéter, mais en contrepartie, on perd en lisibilité. **A vous de choisir !**
:::
